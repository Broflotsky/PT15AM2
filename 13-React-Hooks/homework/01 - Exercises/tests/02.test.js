...
describe("01 | Ejercicios", () => {
  ...
  describe("Actions", () => {
    it('La action enviarForm deberia retornar un objeto con el type y payload de la action', () => {
      expect(actions.enviarForm(undefined)).toEqual({
        type: 'FORM_DATA', payload: undefined
      });
      expect(actions.enviarForm({
  ...
  describe("Reducer", () => {
    it('Deberia retornar el estado global con el payload agregado cuando el caso es FORM_DATA', () => {
      const state = {
        formulario: {}
      };
      const payload = {
    ...
    it('Deberia retornar el estado cuando el caso es default', () => {
  ...
  describe("Dispatch enviarForm", () => {
    it("Deberia invocarse el hook dispatch para poder despachar la action 'enviarFormulario'", () => {
      ...
      // Ahora la action deberia recibir el estado local!
