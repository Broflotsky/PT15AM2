...
describe('05 | Ejercicios', () => {
  ...
  it('Debe renderizar un div con la propiedad "ref" haciendo referencia a useRef, con el texto "TELEFONO + <estadoLocal>"', () => {
    const useStateSpy = jest.spyOn(React, "useState");
    shallow(<CopyData />);
    expect(copyData.find('div').at(2).props().children.join('').includes(useStateSpy.mock.calls[0])).toEqual(true);
  });

  it('El button debe recibir dentro del evento "onClick", la funcion "handleCopy"', () => {
