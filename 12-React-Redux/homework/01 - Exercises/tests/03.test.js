...
describe("03 | Ejercicios", () => {
  it("MapDispatchToProps deberia retornar las actions para dispatchear", () => {
    const dispatch = jest.fn();
    mapDispatchToProps(dispatch).addProduct();
    expect(dispatch.mock.calls[0][0]).toEqual({ type: 'ADD_PRODUCT', payload: undefined });
    // Recordá que debe recibir product por parametros
    mapDispatchToProps(dispatch).addProduct({ name: 'Peras' });
    expect(dispatch.mock.calls[1][0]).toEqual({ type: 'ADD_PRODUCT', payload: { name: 'Peras' } });
  });

  it("handleSubmit deberia hacer dispatch de la action addProduct recibida por props", () => {
  ...
  it('Deberian enviarse los datos del estado local al hacer dispatch de addProduct', () => {
    const addProduct = jest.fn();
    ...
    fireEvent.change(nameInput, { target: { name: 'name', value: 'Naranjas' }});
